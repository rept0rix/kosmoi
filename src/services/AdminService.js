...
export const AdminService = {
    ...
    getMapLocations: async () => {
        try {
            ...
            const response = await fetch(`${supabaseUrl}/rest/v1/service_providers?select=id,business_name,current_lat,current_lng,email,category&order=created_at.desc`, {
                headers: {
                    'apikey': supabaseKey,
    ...
    getUserDetails: async (userId) => {
        try {
            const [userRes, businessRes, walletRes] = await Promise.all([
                ...
                supabase.from('service_providers').select('*').eq('owner_id', userId).maybeSingle(),
     ...
     * Generates a Magic Link using the Service Role Key (exposed in .env for this project)
     * and returns the action link.
     */
    impersonateUser: async (email) => {
        try {
            console.log("AdminService: Impersonating user", email);
            const supabaseUrl = import.meta.env.VITE_SUPABASE_URL;
            const serviceKey = import.meta.env.VITE_SUPABASE_SERVICE_ROLE_KEY;

            if (!serviceKey) throw new Error("Missing Service Role Key");

            // Direct fetch to bypass 'Forbidden use of secret API key in browser' check
