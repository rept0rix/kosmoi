...
export const Core = {
  ...
  CreatePaymentLink: async ({
    ...
    console.log(
      `[Integrations] Creating payment link for ${name} ($${amount}) via Edge Function...`,
    );
  ...
  InvokeAmadeus: async ({ action, params }) => {
    console.log(`[Integrations] Invoking Amadeus Action: ${action}...`);
    try {
      // @ts-ignore
      const { data, error } = await supabase.functions.invoke("amadeus-query", {
        body: { action, params },
      });
      ...
      console.error("Amadeus Error (Edge):", error);
      return { error: error.message };
    }
  },
  ...
  SendN8NEmail: async ({ to, subject, body, campaignId }) => {
    const { n8nService } =
      await import("../services/integrations/N8NService.js");
...
export const InvokeAmadeus = Core.InvokeAmadeus;
