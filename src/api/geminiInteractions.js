 ...
 */
export const callAgentInteraction = async ({
  model,
  prompt,
  system_instruction,
  jsonSchema,
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt++) {
      try {
        ...
        if (isRateLimit && attempt < 2) {
          console.warn(
            `⚠️ Gemini Rate Limit Exceeded (Attempt ${attempt + 1}/3). Waiting 65s...`,
          );
          await new Promise((resolve) => setTimeout(resolve, 65000)); // Wait 65 seconds
          continue;
        }
        throw err; // Other errors or last attempt throw immediately
      }
    }
    ...
    try {
      ...
      const jsonMatch =
        output.text.match(/```json\s*([\s\S]*?)\s*```/) ||
        output.text.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        try {
      ...
      console.warn(
        "Failed to parse JSON from structured output, returning raw text wrapper.",
      );
