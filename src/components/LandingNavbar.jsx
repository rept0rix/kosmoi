...
const LandingNavbar = () => {
  ...
  const isDarkHeroPage =
    ...
    location.pathname.endsWith("/pricing"); /* Simple check */
  ...
  const textColorClass = isTextWhite
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "py-2 bg-white/90 dark:bg-slate-950/90 backdrop-blur-md shadow-sm border-b border-gray-200 dark:border-gray-800" : "py-4 bg-transparent"}`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8 rtl:space-x-reverse">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger
                className={`flex items-center gap-1 text-sm font-medium transition-colors focus:outline-none ${textColorClass}`}
              >
            ...
            {isAuthenticated ? (
              ...
              <>
                ...
                <Button
                  ...
                  variant="ghost"
                  size="sm"
                  className={isTextWhite ? "text-white hover:bg-white/10" : ""}
                >
          ...
          <div className="md:hidden flex items-center gap-2">
            ...
            <Sheet>
              <SheetTrigger asChild>
                ...
                >
                  <Menu
                    className={`w-6 h-6 ${isTextWhite ? "text-white" : "text-gray-700 dark:text-gray-200"}`}
                  />
                </Button>
              </SheetTrigger>
                  ...
                  <div className="flex flex-col gap-2">
                    ...
                    {categories.map((cat) => (
                      <Link
                        key={cat.href}
                        to={cat.href}
                        className="text-base py-1 hover:text-blue-600"
                      >
                        {cat.name}
                      </Link>
                    ))}
