...
export default function Login() {
  ...
  // 1. OAuth Hash Handling - Active Verification
  useEffect(() => {
    let timeoutTimer;

    const verifyOAuth = async () => {
      // Check if we have an access token in URL
      if (location.hash && location.hash.includes("access_token")) {
        // Prevent React Strict Mode double-invocation
        if (processingOAuth.current) {
          console.log(
            "🔒 OAuth processing already in progress. Skipping duplicate.",
          );
          return;
        }
        ...
        console.log(
          "🔐 OAuth Hash Detected - Attempting Manual Parse FIRST...",
        );
        setVerifying(true);
        ...
        try {
          ...
          if (accessToken && refreshToken) {
            ...
            if (data?.session) {
              ...
              checkAppState();
              return; // Stop execution here, we are good
            }
          }
        } catch (e) {
          console.warn(
            "Manual setSession failed, falling back to auto-detection:",
            e,
          );
        }

        // 2. Fallback to Standard Detection (if manual failed)
        try {
          const { data, error } = await db.auth.getSession();
          if (error) throw error;

          if (data?.session) {
            console.log("✅ Auto-Detect Success: Session found.");
            await checkAppState();
          } else {
            console.warn("⚠️ No session found after checks.");
          }
        } catch (e) {
          // Ignore AbortError if we are just checking status
          if (e.name !== "AbortError" && !e.message?.includes("aborted")) {
            console.error("OAuth Verification Error:", e);
            setError("Authentication failed. Please try again.");
            setVerifying(false);
          } else {
            console.log(
              "⚠️ Standard check aborted (likely race), but manual check might have succeeded.",
            );
          }
        }
  ...
  const handleRedirect = () => {
    try {
      ...
      const isDefaultUrl =
        returnUrl === "/board-room" || returnUrl.includes("board-room");
  ...
  const handleAuthSubmit = async (e) => {
    ...
    try {
      ...
      if (session) {
        ...
        console.warn(
          "❌ Login reported success but no session found in post-check.",
        );
        ...
        setTimeout(async () => {
          ...
          if (retrySession) {
            ...
            setError(
              "Login succeeded but session failed to persist. Please retry.",
            );
            setLoading(false);
          }
        }, 1000);
      ...
      if (err.message && err.message.toLowerCase().includes("rate limit")) {
        setError(
          "Too many attempts. Please wait a moment or try a different email.",
        );
      } else {
        setError(
  ...
  const handleForgotPassword = async (e) => {
    e.preventDefault();
    const emailToReset =
      email || prompt("Please enter your email address for password reset:");
  ...
  return (
    <div
      className="min-h-screen relative flex items-center justify-center font-sans overflow-hidden"
      dir="ltr"
    >
        ...
        <div className="flex-1 flex flex-col justify-center text-white space-y-2 md:space-y-8">
          <div>
            ...
            <p className="text-xs md:text-lg text-slate-200/90 font-light max-w-md">
              {isLogin
                ? "Manage your bookings, explore new deals, and chat with your AI concierge."
            ...
            {isLogin
              ? [
                  ...
                  },
                  {
                    icon: MapPin,
                    text: "Your Favorites",
                    sub: "Access your saved spots and itineraries.",
                  },
                  {
                    icon: ShieldCheck,
                    text: "Secure Account",
              ...
              : [
                  {
                    icon: ShieldCheck,
                    text: "Verified Listings & Services",
                    sub: "No scams, just quality.",
                  },
                  {
                    icon: Sparkles,
                    text: "AI Travel Concierge",
                    sub: "Itinerary planning in seconds.",
                  },
                  {
                    icon: MapPin,
                    text: "Exclusive Local Deals",
                    sub: "Prices you won't find online.",
                  },
                ].map((item, i) => (
                  <div
                    key={i}
              ...
              >
                {isLogin ? (
                  <>
                    ...
                    <span className="text-blue-300 font-medium">Sign Up</span>
                  </>
                ) : (
                  <>
                    Already have an account?{" "}
                    <span className="text-blue-300 font-medium">Sign In</span>
                  </>
                )}
              </button>
            </div>
