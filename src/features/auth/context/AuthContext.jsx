...
export const AuthProvider = ({ children }) => {
  ...
  const checkUserAuth = React.useCallback(
    async (hasOptimisticSession, runInBackground = false) => {
      try {
        ...
        const fetchUserAndRole = async () => {
          ...
          if (roleData) {
            ...
            console.warn(
              "Failed to fetch user role, defaulting to user",
              roleError,
            );
          }
        ...
        if (authenticatedUser) {
          ...
          if (hasOptimisticSession) {
            console.warn(
              "⚠️ User fetch failed but optimistic session exists. Retaining session.",
            );
        ...
        if (isAbortError) {
          ...
          console.warn(
            "⚠️ Auth check failed but keeping optimistic session (Stability Mode). Error:",
            error.message,
          );
        } else if (error.message?.includes("timed out")) {
          console.warn(
            "⚠️ Auth validation timed out. Assuming offline/slow network. NOT logging out.",
          );
          ...
          console.warn(
            "⚠️ Unknown auth error, preserving state for stability:",
            error,
          );
  ...
  const checkAppState = React.useCallback(async () => {
    ...
    try {
      ...
      if (
        import.meta.env.DEV &&
        navigator.userAgent.includes("ScreenshotAgent")
      ) {
      ...
      const {
        data: { session },
      } = await Promise.race([getSessionPromise, sessionTimeoutPromise]);
