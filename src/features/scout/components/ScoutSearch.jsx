  ...
  const handleSend = async () => {
    ...
    if (isDeepResearch) {
      try {
        ...
        const url = `${supabaseUrl}/functions/v1/deep-research`;
        ...
        const progressMsgId = Date.now().toString() + "_progress";
        setMessages((prev) => [
          ...prev,
          {
            id: progressMsgId,
            role: "assistant",
            content: "🔄 **Initializing Deep Research...**",
          },
        ]);
            ...
            if (eventMatch && dataMatch) {
              ...
              if (event === "status") {
                ...
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === progressMsgId
                      ? { ...m, content: `🔍 **Searching:** ${data.query}` }
                      : m,
                  ),
                );
                ...
                setMessages((prev) =>
                  prev.map((m) =>
                    m.id === progressMsgId
                      ? { ...m, content: `📖 **Reading:** ${data.title}` }
                      : m,
                  ),
                );
              } else if (event === "plan") {
                setMessages((prev) => [
                  ...prev.filter((m) => m.id !== progressMsgId),
                  {
                    id: Date.now().toString(),
                    role: "assistant",
                ...
                if (!data.sufficient) {
                  setMessages((prev) => [
                    ...prev,
                    {
                      id: Date.now().toString(),
                      role: "assistant",
                      content: `💡 **Insight:** ${data.reason}\n🔄 **Expanding search:** ${data.newQuery}`,
                    },
                  ]);
                }
                ...
                setMessages((prev) => [
                  ...prev.filter((m) => m.id !== progressMsgId),
                  {
                    ...
                    content:
                      data.answer +
                      `\n\n---\n📚 **${data.sourcesRead} sources** • 🔍 **${data.queriesUsed} queries**`,
                  },
                ]);
                setIsTyping(false);
              }
        ...
        setMessages((prev) => [
          ...prev,
          {
            ...
            role: "assistant",
            content:
              "⚠️ **Deep Research Failed**\n\nPossible causes:\n- Missing **SERPER_API_KEY** (get free at serper.dev)\n- Network/server issue\n\nTry again or switch to standard mode.",
          },
        ]);
        setIsTyping(false);
      }
