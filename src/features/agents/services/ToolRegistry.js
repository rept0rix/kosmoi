...
import { AmadeusService } from "@/services/integrations/AmadeusService.js";
...
export const ToolRegistry = {
  ...
  search_services: async (input) => {
    ...
    console.log(
      `[ToolRegistry] Searching Services (Real DB): ${query} in ${location}`,
    );

    try {
      // Use AdminService to fetch real data
      // We'll search by text filter.
      const { data } = await AdminService.getBusinessesPage(1, 10, {
        search: query,
      });
  ...
  insert_interaction: async (input) => {
    ...
    // Call Real Service
    const result = await SalesService.createInteraction(
      input.lead_id,
      input.type || "note",
      input.content,
    );
  ...
  publish_post: async (input) => {
    ...
    // 2. Publish
    const result = await MarketingService.publishPost(
  ...
  get_available_slots: async (input) => {
    ...
    console.log(
      `[ToolRegistry] Checking slots for ${input.provider_id} on ${input.date}`,
    );
  ...
  // --- HOTEL API TOOLS (Amadeus) ---
  search_external_hotels: async (input) => {
    // input: { cityCode: 'USM' }
    console.log(`[ToolRegistry] Searching External Hotels (Amadeus)...`);
    const data = await AmadeusService.searchHotels(input.cityCode || "USM");
    if (!data || data.length === 0)
      return "No external hotels found for this city.";

    return data.map((h) => ({
      hotel_id: h.hotelId,
      name: h.name,
      iata_code: h.iataCode,
      chain_code: h.chainCode,
      latitude: h.geoCode?.latitude,
      longitude: h.geoCode?.longitude,
    }));
  },

  get_hotel_quotes: async (input) => {
    // input: { hotelIds: '...', adults: 1, checkInDate: 'YYYY-MM-DD', checkOutDate: 'YYYY-MM-DD' }
    console.log(`[ToolRegistry] Getting Hotel Quotes (Amadeus)...`);
    const data = await AmadeusService.getHotelOffers(input.hotelIds, {
      adults: input.adults,
