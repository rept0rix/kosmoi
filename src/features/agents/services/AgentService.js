...
console.log(
  "AgentService Module Loaded. Tools Registered:",
    ...
    if (options.approved) {
      ...
      const validUserId =
        userId && isUuid(userId)
          ? userId
          : "2ff0dcb1-37f2-4338-bb3b-f71fb6dd444e"; // Fallback Dev ID
...
Approval ID: ${approvalData.id}`;
    ...
    try {
      ...
      if (query) {
        queryBuilder = queryBuilder.or(
          `problem_pattern.ilike.%${query}%,solution_pattern.ilike.%${query}%`,
        );
      }
  ...
  // --- LOGGING & EXECUTION ---
  Logger.info("AgentService", `Executing tool: ${toolName}`, {
    agentId,
    payload,
  });
  ...
  try {
    ...
    Logger.success("AgentService", `Tool executed: ${toolName}`, {
      agentId,
      result:
    ...
    Logger.error("AgentService", `Tool failed: ${toolName}`, {
      agentId,
      error: error.message,
    });
    throw error;
...
export class AgentService {
  ...
  async sendMessage(message, options = {}) {
    ...
    if (!this.rateLimiter.take(1)) {
      // Silently ignore to avoid spamming the user
      return {
        text: "", // Silently ignore rate limit
