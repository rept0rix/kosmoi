...
export function RegisterBusinessForm({ initialName = "", onBack, onSuccess }) {
  ...
  const createBusinessMutation = useMutation({
    mutationFn: async (/** @type {any} */ businessData) => {
      ...
      // 1. Create the Service Provider Entity
      const newProvider = await db.entities.ServiceProvider.create({
        ...businessData,
        owner_id: user.id,
      });

      // 2. Critical Fix: Upgrade User Role to Vendor
      try {
        // A. Update Public Profile (Explicitly set role to verify)
        if (user?.id) {
          await db.from("users").update({ role: "vendor" }).eq("id", user.id);
        }

        // B. Update Auth Metadata
        // @ts-ignore
        if (db.auth.updateMe) {
          // @ts-ignore
          await db.auth.updateMe({ role: "vendor" });
        }

        console.log("User role upgraded to Vendor");
      } catch (roleError) {
        console.warn("Failed to upgrade user role:", roleError);
        // Don't fail the business creation if role update fails, but warn
      }

      return newProvider;
    },
    onSuccess: async () => {
      // 3. Force Refresh Auth State so UI updates immediately
  ...
  const isStep1Valid = () =>
    formData.business_name.length > 1 &&
    formData.contact_name.length > 1 &&
                  ...
                  <div className="space-y-2 relative">
                    ...
                    {!mapPosition && showMap && (
                      <p className="text-xs text-amber-600 mt-1">
                        Please tap on the map to mark the exact location.
                      </p>
                    )}
                  </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-300">
              <div className="text-center space-y-2">
                ...
                <p className="text-sm text-slate-500">
                  Upload a logo/profile picture and a gallery to showcase your
                  business.
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-3 sm:grid-cols-4 gap-4">
                  {galleryImages.map((url, i) => (
                    <div
                      key={i}
                      className="relative aspect-square group rounded-lg overflow-hidden border bg-slate-50"
                    >
                      <img
                        src={url}
                        alt={`Gallery ${i}`}
              ...
              <div className="flex justify-between mt-8 pt-4">
                ...
                <Button
                  ...
                  className="ml-auto mr-2 text-slate-500 hover:text-slate-700"
                >
                  Skip for now
                </Button>
                <Button
