...
export function ClaimBusinessFlow({
  ...
  const claimMutation = useMutation({
    mutationFn: async (/** @type {any} */ placeData) => {
      ...
      if (!user?.id)
        throw new Error("User authentication missing during claim");
      ...
      if (existing) {
        ...
        if (existing.owner_id) {
          throw new Error(
            "This business is already registered by another user.",
          );
        }
        ...
        const { data: updated, error: updateError } = await db
          ...
          .update({
            owner_id: user.id,
            status: requirePayment ? "pending_verification" : "verified",
            verified: !requirePayment,
            stripe_status: requirePayment ? "unpaid" : "verified",
      ...
      const payload = {
        ...
        status: requirePayment ? "pending_verification" : "active", // Set active immediately unless payment required
        verified: !requirePayment,
        stripe_status: requirePayment ? "unpaid" : "verified",
        ...
        metadata: {
          ...
          google_photos:
            placeData.photos?.map((p) =>
              typeof p.getUrl === "function" ? p.getUrl() : p,
            ) || [],
        },
        owner_id: user?.id,
        claimed_at: new Date().toISOString(),
        ...
        <CardContent className="p-8 text-center space-y-6">
          ...
          <div className="space-y-2">
            ...
            {selectedPlace.international_phone_number && (
              <p className="font-mono text-sm bg-slate-100 inline-block px-3 py-1 rounded-full">
                {selectedPlace.international_phone_number}
              </p>
            )}
          </div>
