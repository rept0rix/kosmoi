...
export default function BusinessDashboard() {
  ...
  useEffect(() => {
    if (!isLoadingUser && !user && !localStorage.getItem("sb-access-token")) {
      ...
      console.warn(
        "⚠️ No user found in BusinessDashboard. Redirecting to login...",
      );
  ...
  useEffect(() => {
    ...
    if (paymentSuccess) {
      toast({
        ...
        description:
          "Your subscription has been updated. Welcome to the new tier!",
        className: "bg-green-600 text-white border-none",
        duration: 5000,
      });
      // Optionally remove the param from URL to prevent toast on refresh
  ...
  }, [isLoadingBusinesses, myBusinesses, searchParams, setSearchParams]);
            ...
            <Card className="border-dashed border-2 border-slate-300 bg-slate-50/50">
              <CardContent className="flex flex-col items-center justify-center py-16 text-center">
                ...
                <p className="text-slate-500 mb-6 max-w-md">
                  Start by adding your first business. You can claim an existing
                  Google Maps profile or create a new one.
                </p>
                <Button
                  onClick={handleStartAdd}
                  size="lg"
            ...
            <div className="grid md:grid-cols-2 gap-6">
              {myBusinesses?.map((bus) => (
                <Card
                  ...
                  onClick={() => handleBusinessSelect(bus.id)}
                >
      ...
      <div className="min-h-screen bg-slate-50 p-6 flex items-center justify-center">
        <Card className="w-full max-w-2xl">
          <CardContent className="p-8">
            ...
            >
              <Search className="w-4 h-4 mr-2" /> Back into Hub
            </Button>
            <BusinessSearchStep
  ...
  if (viewMode === "dashboard" && selectedBusinessId) {
    const activeBusiness = myBusinesses.find(
