...
export default function AgentChat() {
    ...
    const handleSendMessage = async () => {
        ...
        try {
            ...
            // Handle Actions
            if (response.action && response.action.type === 'create_task') {
                try {
                    ...
                    const { error } = await supabase.from('agent_tasks').insert([{
                        ...
                        status: 'in_progress',
                        created_by: 'consumer-chat' // origin
                    }]);

                    if (error) throw error;

                    // System Message confirming action
                    const sysMsg = {
                        id: (Date.now() + 2).toString(),
                        role: 'system',
                        agent_id: 'SYSTEM',
                        content: `**Action Started:** ${response.action.title}\n> ${response.action.description}`,
                        created_at: new Date().toISOString()
                    };
