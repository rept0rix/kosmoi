...
const BoardRoomHeader = ({
    ...
    const handleSyncLinear = async () => {
        const apiKey = import.meta.env.VITE_LINEAR_API_KEY || prompt("Enter Linear API Key:");
        if (!apiKey) return;
        const teamKey = prompt("Enter Team Key (e.g. KOS) [Optional]:") || null;

        try {
            const response = await fetch('http://127.0.0.1:8003/integrations/linear/sync', {
                method: 'POST',
            ...
            if (result.status === 'success') {
                ...
                for (const issue of issues) {
                    await handleCreateTask({
                        ...
                        description: issue.description || `Imported from Linear: ${issue.url}`,
                        priority: 'medium'
                    });
                    createdCount++;
                }

                alert(`Successfully imported ${createdCount} issues from Linear into the Board Room!`);
            } else {
                alert('Sync failed: ' + result.message);
            }
        } catch (e) {
            console.error(e);
            alert('Failed to connect to Integration Hub. Is it running? (cd src/knowledge/n8n-workflows/src && python3 integration_hub.py)');
        }
    };
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center p-4 md:p-6 border-b border-white/10 bg-white/80 dark:bg-black/40 backdrop-blur-xl sticky top-0 z-10 gap-4 md:gap-6 shadow-sm">
                        ...
                        {meetings.map(meeting => (
                            <SelectItem key={meeting.id} value={meeting.id} className="py-4">
                                <div className="flex flex-col items-start gap-1">
                                    <span className="font-bold text-base dark:text-slate-200">{meeting.title}</span>
                                    <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
                                        <span>{new Date(meeting.created_at).toLocaleDateString()}</span>
                                        {meeting.status === 'archived' && <Badge variant="outline" className="text-xs dark:border-slate-700 dark:text-slate-400">Archived</Badge>}
                                    </div>
                                </div>
                            </SelectItem>
                        ))}
            ...
            <div className="hidden md:flex items-center gap-3">
                ...
                <Select onValueChange={handleStartWorkflow}>
                    ...
                    <SelectContent className="dark:bg-slate-900 dark:border-slate-800">
                        {workflows?.map(wf => (
                            <SelectItem key={wf.id} value={wf.id} className="dark:text-slate-300 dark:focus:bg-slate-800 py-3 text-base">
                                {wf.name}
                            </SelectItem>
                        ))}
                    </SelectContent>
            ...
            <div className="flex items-center gap-3 ml-auto">
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">{activeAgentsCount} Agents</span>
                            </Button>
                        </TooltipTrigger>
                        <TooltipContent>
                            <p>Manage Agents</p>
                    ...
                    <Tooltip>
                        <TooltipTrigger asChild>
                            ...
                            >
                                {localBrainEnabled ? (
                                    <>
                                        <div className={`w-2.5 h-2.5 rounded-full ${localBrainStatus?.isReady ? 'bg-green-300' : 'bg-yellow-300 animate-pulse'}`} />
                ...
                <Button
                    ...
                    className={`h-10 px-4 gap-2 text-base rounded-lg transition-all duration-500 border-2 ${autonomousMode
                ...
                >
                    {autonomousMode ? <Pause className="w-5 h-5" /> : <Play className="w-5 h-5" />}
                    <span className="hidden sm:inline font-bold">{autonomousMode ? "AUTONOMOUS" : "Auto Pilot"}</span>
                </Button>
                ...
                <div className="pl-3 border-l border-slate-200 ml-2 dark:border-slate-800">
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="relative h-12 w-12 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors" aria-label="User Menu">
                                <Avatar className="h-11 w-11 border-2 border-white dark:border-slate-800 shadow-sm ring-2 ring-indigo-50 dark:ring-indigo-900/30">
                                    ...
                                    <AvatarFallback className="bg-indigo-600 text-white font-bold text-lg">NY</AvatarFallback>
                                </Avatar>
