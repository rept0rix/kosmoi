...
const MOCK_EXPERIENCES_DETAILS = {
  "exp-1": {
    ...
    reviews_count: 124,
    images: [
      "https://images.unsplash.com/photo-1534008897995-27a23e859048?w=1600&q=80",
      "https://images.unsplash.com/photo-1590523277543-a94d2e4eb00b?w=800&q=80",
    ],
    description:
      "Explore the 42 islands of Ang Thong National Marine Park. Snorkel in crystal clear waters, hike to the Emerald Lake, and kayak along limestone cliffs. A full day of adventure and natural beauty.",
  ...
  "exp-2": {
    ...
    description:
      "Master the art of Thai cuisine with our expert local chefs. Visit the local market to pick fresh ingredients, then learn to cook 4 classic dishes from scratch.",
  ...
  default: {
    description:
      "Experience the best of Koh Samui with this curated activity. Enjoy local culture, breathtaking views, and professional service.",
    schedule: "Flexible",
    inclusions: [
      "Professional Guide",
...
export default function ExperienceDetails() {
    ...
    const fetchExperience = async () => {
      ...
      try {
        ...
        if (data && !error) {
          ...
          // 2. Fallback to Mock
          const mock =
            MOCK_EXPERIENCES_DETAILS[id] ||
            MOCK_EXPERIENCES_DETAILS[
              Object.keys(MOCK_EXPERIENCES_DETAILS).find((k) =>
                k.startsWith(id),
              )
            ] ||
            null;
  ...
  const handleBookingSubmit = async (formData) => {
    // formData from BookingDialog: { date, time, guests, message, name, contact }
    const contactName = formData.name || formData.contact || "Guest";
    const contactEmail =
    ...
    // If generic contact info was put in "contact" field
    ...
    try {
      ...
      const leadData = {
        ...
        email: contactEmail || "no-email@provided.com", // CRM might require email
        phone: contactPhone,
        notes: `Deep Booking Request: ${experience?.title}\nDate: ${formData.date ? formData.date.toDateString() : "Flexible"}\nGuests: ${formData.guests}\nMessage: ${formData.message}\nContact Info: ${formData.contact}`,
        source: "Experience Details",
        value: experience?.price ? experience.price * formData.guests : 0,
      };
                ...
                <CardContent className="p-6 md:p-8">
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                    <div>
                      <Badge
                        variant="secondary"
                        className="mb-2 uppercase tracking-wide text-xs font-semibold text-primary bg-primary/10 hover:bg-primary/20"
                      >
                        {t(`experiences.category_${experience.category}`) ||
                          experience.category}
                      </Badge>
                      <h1 className="text-3xl md:text-4xl font-bold text-gray-900 font-display mb-2">
                        {experience.title}
                      </h1>
                      <div className="flex items-center text-gray-500 text-sm gap-4">
                        ...
                        <span className="flex items-center">
                          ...
                          {experience.rating} ({experience.reviews_count}{" "}
                          reviews)
                        </span>
                      </div>
                    </div>
                ...
                <Card className="border-none shadow-xl rounded-2xl overflow-hidden ring-1 ring-gray-100">
                  <CardContent className="p-6">
                    <div className="flex items-end gap-2 mb-6">
                      ...
                      <span className="text-gray-500 mb-1.5">
                        {t("experiences.price_person")}
                      </span>
                    </div>
                    ...
                    <div className="pt-4 border-t border-gray-100 mb-6">
                      <div className="flex justify-between items-center text-sm mb-2">
                        <span className="text-gray-600">
                          {formatPrice(experience.price)} x {inquiryForm.guests}
                        </span>
                        <span className="font-semibold">
                          {formatPrice(experience.price * inquiryForm.guests)}
                        </span>
                      </div>
                    ...
                    <BookingDialog
                      ...
                      provider={{
                        ...experience,
                        business_name: experience.title,
                      }}
                      mode="inquiry"
