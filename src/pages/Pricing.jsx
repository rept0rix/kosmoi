...
const Pricing = () => {
  ...
  const [isProcessing, setIsProcessing] = React.useState(null); // 'trial', 'growth', 'scale'

  const handleCheckout = async (planId, price) => {
    if (!isAuthenticated) {
      toast({
        title: "Log in required",
        description: "Please log in to upgrade your business.",
        variant: "default",
      });
    ...
    try {
      const result = await CreatePaymentLink({
        name: `Kosmoi ${planId} Plan`,
        amount: parseFloat(price),
        currency: "thb",
        success_url: `${window.location.origin}/businessdashboard?payment_success=true&plan=${planId}`,
        cancel_url: window.location.href,
        metadata: {
          userId: user?.id, // Ensure user ID is passed if available
          planId: planId,
        },
      });
      ...
      toast({
        title: "Payment Error",
        description:
          error.message || "Something went wrong initiating payment.",
        variant: "destructive",
      });
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white pt-32 pb-20 font-sans relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <h1 className="text-4xl md:text-6xl font-extrabold mb-6 tracking-tight text-white drop-shadow-xl font-['Outfit']">
            Simple,{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-amber-400 to-amber-600 animate-pulse">
              Transparent
            </span>{" "}
            Pricing
          </h1>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
          {/* Free Plan */}
          <div className="glass-card-premium p-6 md:p-8 rounded-3xl flex flex-col hover:border-amber-500/30 transition-all duration-300">
            <div className="mb-8">
              <h3 className="text-lg font-bold text-slate-300 mb-2 font-['Outfit']">
                Basic
              </h3>
              ...
              <p className="text-slate-500 mt-4 text-sm">
                Essential visibility for every business.
              </p>
            </div>
          ...
          {/* 35 THB Test Drive */}
          <div className="glass-card-premium p-6 md:p-8 rounded-3xl flex flex-col border-amber-400/30 shadow-amber-500/20 hover:scale-[1.02] transition-all relative overflow-hidden group">
            <div className="grainy-noise absolute inset-0 opacity-20"></div>
            <div className="absolute top-0 right-0 bg-gradient-to-l from-amber-500 to-amber-400 text-slate-950 text-[10px] font-bold px-3 py-1 rounded-bl-xl z-20">
              POPULAR
            </div>
            <div className="mb-8 relative z-10">
              <h3 className="text-lg font-bold text-amber-200 mb-2 font-['Outfit']">
                Test Drive
              </h3>
              <div className="flex items-baseline gap-1">
              ...
              <p className="text-slate-400 mt-4 text-sm">
                See the real impact immediately.
              </p>
            </div>
            <ul className="space-y-4 mb-8 flex-1 relative z-10">
              {[
                {
                  t: "Verified Status (2 Weeks)",
                  d: "Blue checkmark to build trust",
                },
            ...
            <button
              onClick={() => handleCheckout("Test Drive", "35")}
              disabled={!!isProcessing}
              className="relative z-10 w-full py-3 px-4 bg-gradient-to-r from-amber-500 to-amber-600 text-slate-950 rounded-xl font-bold hover:shadow-lg hover:shadow-amber-500/40 transition-all font-['Outfit'] tracking-wide disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isProcessing === "Test Drive" ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : null}
              {isProcessing === "Test Drive" ? "Processing..." : "Try for ฿35"}
            </button>
          </div>
          ...
          <div className="glass-card-premium p-6 md:p-8 rounded-3xl flex flex-col hover:border-blue-400/30 transition-all duration-300">
            <div className="mb-8">
              <h3 className="text-lg font-bold text-blue-200 mb-2 font-['Outfit']">
                Growth
              </h3>
              <div className="flex items-baseline gap-1">
              ...
              <p className="text-slate-400 mt-4 text-sm">
                Consistent leads for steady growth.
              </p>
            </div>
            <ul className="space-y-4 mb-8 flex-1">
              {[
                ...
                {
                  t: "Up to 50 Leads / mo",
                  d: "Direct connections to customers",
                },
              ].map((item, i) => (
                <li key={i} className="flex flex-col gap-1">
                  <div className="flex items-center gap-3 text-slate-200 font-medium text-sm">
            ...
            <button
              onClick={() => handleCheckout("Growth", "1500")}
              disabled={!!isProcessing}
              className="w-full py-3 px-4 bg-blue-600/90 text-white rounded-xl font-bold hover:bg-blue-600 transition-colors shadow-lg shadow-blue-900/20 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isProcessing === "Growth" ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : null}
              {isProcessing === "Growth" ? "Processing..." : "Choose Growth"}
            </button>
          </div>
          ...
          <div className="glass-card-premium p-6 md:p-8 rounded-3xl flex flex-col relative transform md:-translate-y-4 border-purple-500/30 bg-purple-900/10 hover:bg-purple-900/20 transition-all">
            ...
            <div className="mb-8">
              <h3 className="text-lg font-bold text-purple-200 mb-2 font-['Outfit']">
                Scale
              </h3>
              <div className="flex items-baseline gap-1">
                <span className="text-4xl font-extrabold text-white">
              ...
              <p className="text-slate-400 mt-4">
                Dominate your niche with AI power.
              </p>
            </div>
            <ul className="space-y-4 mb-8 flex-1">
              {[
                {
                  t: "Priority Search Listing",
                  d: "Always appear first in results",
                },
                ...
                { t: "Unlimited Leads", d: "Never miss a potential customer" },
                {
                  ...
                  d: "Connect directly to your phone",
                },
                {
                  t: "Custom Promo Campaigns",
                  d: "Push notifications to nearby users",
                },
              ].map((item, i) => (
                <li key={i} className="flex flex-col gap-1">
                  <div className="flex items-center gap-3 text-white font-medium text-sm">
            ...
            <button
              onClick={() => handleCheckout("Scale", "3500")}
              disabled={!!isProcessing}
              className="w-full py-3 px-4 bg-purple-600 text-white rounded-xl font-bold hover:bg-purple-500 transition-colors shadow-lg shadow-purple-500/25 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {isProcessing === "Scale" ? (
                <Loader2 className="w-5 h-5 animate-spin" />
              ) : null}
              {isProcessing === "Scale" ? "Processing..." : "Go Premium"}
            </button>
          </div>
        </div>
        ...
        <div className="mt-24 max-w-7xl mx-auto">
          <h2 className="text-3xl font-bold text-white text-center mb-12 font-['Outfit']">
            Compare Plans
          </h2>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-white/10 bg-white/5">
                  ...
                  <th className="p-6 text-white font-bold text-center">
                    Basic
                    <br />
                    <span className="text-slate-500 font-normal opacity-0">
                      Free
                    </span>
                  </th>
                  <th className="p-6 text-amber-400 font-bold text-center bg-amber-500/10">
                    Test Drive
                    <br />
                  ...
                  <th className="p-6 text-blue-400 font-bold text-center">
                    Growth
                    <br />
                    <span className="text-slate-400 font-normal text-sm">
                      1,500฿ / mo
                  ...
                  <th className="p-6 text-purple-400 font-bold text-center">
                    Scale
                    <br />
                    <span className="text-slate-400 font-normal text-sm">
                      3,500฿ / mo
              ...
              <tbody className="divide-y divide-white/5">
                ...
                <tr className="bg-white/5">
                  <td
                    colSpan={5}
                    className="p-4 font-bold text-white text-sm tracking-widest uppercase pl-6"
                  >
                    Visibility
                  </td>
                </tr>
                ...
                <tr>
                  ...
                  <td className="p-4 text-center bg-amber-500/5">
                    <CheckCircle className="w-5 h-5 text-amber-400 mx-auto" />
                  </td>
                  <td className="p-4 text-center">
                ...
                <tr>
                  ...
                  <td className="p-4 text-center bg-amber-500/5">
                    <span className="text-amber-400 font-medium">Boosted</span>
                  </td>
                  <td className="p-4 text-center">
                    <span className="text-blue-400 font-medium">High</span>
                  </td>
                  <td className="p-4 text-center">
                    <span className="text-purple-400 font-bold">Top Tier</span>
                  </td>
                </tr>
                ...
                <tr>
                  <td className="p-4 pl-6 text-slate-300">
                    AI Auto-Reply (Receptionist)
                  </td>
                  <td className="p-4 text-center">
                    <span className="text-slate-600">-</span>
                  </td>
                  <td className="p-4 text-center bg-amber-500/5 text-sm text-amber-200">
                    Demo (1 chat)
                  </td>
                  <td className="p-4 text-center text-sm text-blue-200">
                    Standard
                  </td>
                ...
                <tr>
                  ...
                  <td className="p-4 text-center">
                    <span className="text-slate-600">Basic Views</span>
                  </td>
                  <td className="p-4 text-center bg-amber-500/5 text-sm">
                    2-week History
                  </td>
        ...
        <div className="mt-16 max-w-4xl mx-auto glass-card-premium rounded-3xl p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8 border border-white/10">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-400 max-w-md">
              For hotel chains, large venues, or custom integrations. Get API
              access, dedicated account management, and multi-location support.
            </p>
          </div>
        ...
        <div className="mt-20 text-center">
          <p className="text-slate-500">
            ...
            <span className="font-bold text-white">
              Kosmoi is 100% Free for everyone.
            </span>
          </p>
        </div>
      </div>
