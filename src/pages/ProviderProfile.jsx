...
export default function ProviderProfile() {
  ...
  const loadProvider = async () => {
    ...
    const { data, error } = await db.entities.ServiceProvider.filter({
      id: providerId,
    });
        ...
        <div className="absolute bottom-0 left-0 right-0 p-6 text-white max-w-7xl mx-auto w-full">
          ...
          >
            <div>
              ...
              <h1 className="text-3xl md:text-4xl font-bold mb-1 shadow-sm">
                {provider.business_name}
              </h1>
              <div className="flex items-center text-slate-200 text-sm gap-4">
                <span className="flex items-center gap-1">
                  <MapPin className="w-4 h-4" />{" "}
                  {provider.location || "Samui, Thailand"}
                </span>
                <span className="flex items-center gap-1">
            ...
            <div className="hidden md:flex gap-2">
              {/* Claim Button for Unowned Profiles */}
              {!provider.owner_id && (
                <Button
        ...
        <div className="md:col-span-2 space-y-6">
          ...
          <GlassCard className="p-6 bg-white dark:bg-slate-900 border-0 shadow-sm">
            <h2 className="text-xl font-semibold mb-4 text-slate-800 dark:text-slate-100">
              {t("provider.about") || "About"}
            </h2>
            <p className="text-slate-600 dark:text-slate-300 leading-relaxed">
              {provider.description ||
                "Experience the best service in Koh Samui. We are dedicated to providing top-quality experiences for all our guests. Contact us for more details and specific arrangements."}
            </p>
          ...
          {/* Services / Packages */}
          {provider.price_packages &&
            provider.price_packages.length > 0 /* @ts-ignore */ && (
              <GlassCard className="p-6 bg-white dark:bg-slate-900 border-0 shadow-sm">
                <h2 className="text-xl font-semibold mb-4 text-slate-800 dark:text-slate-100">
                  Services & Pricing
                </h2>
                <div className="space-y-4">
                  {provider.price_packages.map((pkg, i) => (
                    <div
                      key={i}
                      className="flex justify-between items-center p-4 border rounded-xl hover:border-blue-300 bg-slate-50/50 transition-colors"
                    >
                      <div>
          ...
          <span className="font-bold text-lg text-blue-600">
            {provider.price_packages && provider.price_packages.length > 0
              ? new Intl.NumberFormat("th-TH", {
                  ...
                  Math.min(...provider.price_packages.map((p) => p.price)),
                )
              : "Contact for price"}
          </span>
        </div>
