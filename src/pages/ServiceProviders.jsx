...
import HotelSearchExternal from "@/components/HotelSearchExternal";
...
export default function ServiceProviders() {
  ...
  // View State
  const [viewMode, setViewMode] = useState("local"); // 'local' or 'external_hotels'
  ...
  const checkLocation = async () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        try {
          const addresses = await getAddressFromCoordinates(
            location.latitude,
            location.longitude,
          );
          if (addresses.en) {
  ...
  const updateUrlFilters = (newFilters) => {
    ...
    if (newFilters.superCategory !== "all")
      params.set("super_category", newFilters.superCategory);
    else params.delete("super_category");
    if (newFilters.subCategory !== "all")
      params.set("sub_category", newFilters.subCategory);
    else params.delete("sub_category");
    navigate(createPageUrl("ServiceProviders") + `?${params.toString()}`, {
      replace: true,
    });
  };
  ...
  const {
    ...
    queryFn: async () => {
      try {
        const data = await db.entities.ServiceProvider.filter({
          status: "active",
        });
        return data;
      } catch (err) {
  ...
  const filteredProviders = providers
    ...
    .filter((provider) => {
      ...
      const matchesSuperCategory =
        ...
        provider.super_category === filters.superCategory;
      const matchesSubCategory =
        filters.subCategory === "all" ||
        provider.category === filters.subCategory;
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pb-20 transition-colors duration-500">
      ...
      <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-14 z-20 border-b border-gray-200 dark:border-slate-800 transition-all duration-300 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 py-3 space-y-3">
          <div className="flex gap-4 border-b border-gray-100 dark:border-slate-800 pb-2 mb-2">
            <button
              onClick={() => setViewMode("local")}
              className={cn(
                "pb-2 px-1 text-sm font-bold transition-all border-b-2",
                viewMode === "local"
            ...
            <button
              onClick={() => setViewMode("external_hotels")}
              className={cn(
                "pb-2 px-1 text-sm font-bold transition-all border-b-2 font-poppins",
                viewMode === "external_hotels"
                  ? "border-indigo-600 text-indigo-600"
                  : "border-transparent text-gray-400 hover:text-gray-600",
              )}
            >
            ...
            <div className="flex flex-col md:flex-row gap-3 items-center justify-between">
              ...
              <div className="flex flex-wrap gap-2 w-full md:w-auto">
                <Select
                  ...
                  onValueChange={(val) =>
                    handleFilterChange("superCategory", val)
                  }
                >
                ...
                {filters.superCategory !== "all" && (
                  <Select
                    ...
                    onValueChange={(val) =>
                      handleFilterChange("subCategory", val)
                    }
                  >
                    <SelectTrigger className="w-[160px] h-9 text-sm bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700">
              ...
              <div className="flex gap-2 w-full md:w-auto items-center">
                {userLocation && (
                  <div
                    className="hidden md:flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-400 bg-gray-100 dark:bg-slate-800 py-1.5 px-3 rounded-full border border-gray-200 dark:border-slate-700 cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-700 transition-colors"
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        {viewMode === "local" ? (
            ...
            <div className="w-full lg:w-7/12 overflow-y-auto pr-1 custom-scrollbar pb-24">
              <div className="flex justify-between items-center mb-4">
                ...
                <div className="text-xs text-slate-500 flex gap-2">
                  <span
                    className={`cursor-pointer px-2 py-1 rounded-md ${sortBy === "rating" ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}
                    onClick={() => setSortBy("rating")}
                  >
                    Rating
                  </span>
                  <span
                    className={`cursor-pointer px-2 py-1 rounded-md ${sortBy === "distance" ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300" : "hover:bg-slate-100 dark:hover:bg-slate-800"}`}
                      ...
                      <div className="flex flex-col sm:flex-row h-full">
                        ...
                        <div className="sm:w-32 h-32 sm:h-auto bg-slate-200 dark:bg-slate-800 relative">
                          ...
                          <div className="absolute top-2 left-2 sm:hidden">
                            <Badge
                              variant="secondary"
                              className="backdrop-blur-md bg-white/80 dark:bg-black/60 shadow-sm text-xs border-0"
                            >
                        ...
                        <div className="p-4 flex-1 flex flex-col justify-between">
                          ...
                          <div className="mt-4 flex gap-2 justify-end">
                            <Button
                              size="sm"
                              variant="ghost"
                              className="h-8 text-xs hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-300"
                              onClick={(e) => {
                                e.stopPropagation();
                            ...
                            <Button
                              size="sm"
                              className="h-8 text-xs bg-indigo-600 hover:bg-indigo-700 text-white shadow-md shadow-indigo-500/20"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleBook(provider);
                              }}
          ...
          <div className="w-full overflow-y-auto min-h-[calc(100vh-10rem)]">
            <HotelSearchExternal />
          </div>
        )}
      </div>
          ...
          {locationDialogView === "search" ? (
            <div className="space-y-4 py-4">
              <Button
                ...
                className="w-full bg-indigo-50 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 border border-indigo-200 dark:border-indigo-800/50 h-10"
              >
                  ...
                  <div className="space-y-1">
                    {locationHistory.map((loc, idx) => (
                      <Button
                        key={idx}
                        variant="ghost"
                        className="w-full justify-start h-auto py-2 px-2 text-start font-normal hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-300"
            ...
            <div className="space-y-4">
              ...
              <div className="flex gap-2">
                <Button
                  ...
                  className="flex-1 border-slate-200 dark:border-slate-700 dark:text-white dark:hover:bg-slate-800"
                >
                  {t("change")}
                </Button>
                <Button
