        ...
        <div className="h-full flex flex-col p-6 space-y-6 text-slate-100 overflow-y-auto">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-400">Autonomous loop that analyzes metrics and proposes improvements.</p>
                </div>
                <div className="flex items-center gap-4">
                    <div className="flex items-center gap-2 px-4 py-2 bg-slate-800 rounded-full border border-slate-700">
                        <Activity className={`w-4 h-4 ${isRunning ? 'text-green-400 animate-pulse' : 'text-slate-500'}`} />
                        <span className="text-sm font-mono">{isRunning ? 'ACTIVE LOOP' : 'IDLE'}</span>
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl flex flex-col justify-between">
                    <div>
                        ...
                        <p className="text-sm text-slate-400 mb-4">Start or pause the continuous analysis cycle.</p>
                    </div>
                    <Button
                        onClick={toggleLoop}
                    ...
                    >
                        {isRunning ? <><Pause className="w-4 h-4 mr-2" /> Stop Optimizer</> : <><Play className="w-4 h-4 mr-2" /> Start Optimizer</>}
                    </Button>
                </div>
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl">
                    ...
                    <div className="space-y-3 mt-4">
                        {insights.length === 0 ? (
                            <div className="text-sm text-slate-500 italic">No recent optimizations found.</div>
                        ) : (
                            insights.slice(0, 3).map(insight => (
                                <div key={insight.id} className="flex gap-3 items-start p-2 rounded hover:bg-white/5">
                                    {insight.type === 'fix' ? <AlertCircle className="w-4 h-4 text-amber-400 mt-1" /> : <Zap className="w-4 h-4 text-blue-400 mt-1" />}
                                    <div>
                                        <div className="text-sm font-medium">{insight.title}</div>
                                        <div className="text-xs text-slate-500">{new Date(insight.created_at).toLocaleTimeString()}</div>
                                    </div>
                                </div>
                            ))
                        )}
                ...
                <div className="bg-slate-900/50 border border-slate-800 p-6 rounded-xl">
                    ...
                    <div className="grid grid-cols-2 gap-4 mt-4 text-center">
                        <div className="p-3 bg-slate-800/50 rounded-lg">
                            <div className="text-2xl font-bold text-indigo-400">{insights.length}</div>
                            <div className="text-xs text-slate-400 uppercase">Total Insights</div>
                        </div>
                        <div className="p-3 bg-slate-800/50 rounded-lg">
                            <div className="text-2xl font-bold text-green-400">
                                {insights.filter(i => i.status === 'completed' || i.status === 'implemented').length}
                            </div>
            ...
            <div className="flex-1 bg-slate-900/50 border border-slate-800 rounded-xl p-6">
                ...
                <div className="space-y-4">
                    {insights.map(insight => (
                        <div key={insight.id} className="flex items-start gap-4 p-4 bg-slate-800/50 rounded-lg border border-slate-700/50 hover:border-indigo-500/30 transition-colors">
                            <div className={`mt-1 p-2 rounded-lg ${insight.type === 'fix' ? 'bg-amber-500/10 text-amber-400' : 'bg-blue-500/10 text-blue-400'}`}>
                                {insight.type === 'fix' ? <AlertCircle className="w-5 h-5" /> : <Zap className="w-5 h-5" />}
                            </div>
                            <div className="flex-1">
                                <div className="flex justify-between items-start mb-1">
                                    <h4 className="font-medium text-slate-200">{insight.title}</h4>
                                    <span className={`text-xs px-2 py-0.5 rounded-full border ${insight.status === 'pending' ? 'bg-gray-800 text-gray-400 border-gray-700' :
                                            insight.status === 'implemented' ? 'bg-green-900/30 text-green-400 border-green-800' :
                                ...
                                <div className="flex gap-4 text-xs font-mono text-slate-500">
                                    <span>IMPACT: <span className="text-slate-300">{insight.impact}</span></span>
