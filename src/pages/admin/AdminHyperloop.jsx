  ...
  return (
    <div className="p-8 space-y-8 bg-slate-950 min-h-screen text-slate-100 font-sans">
      <div className="flex items-center justify-between">
        ...
        <Badge
          variant="outline"
          className="animate-pulse bg-green-500/10 text-green-400 border-green-500/50"
        >
            ...
            <ScrollArea className="h-[400px] pr-4">
              <div className="space-y-4">
                {crawlerLogs.map((log) => (
                  <div
                    key={log.id}
                    className="p-3 rounded-lg bg-white/5 border border-white/5 flex flex-col gap-1 transition-all hover:bg-white/10"
                  >
                  ...
                  >
                    <div className="flex justify-between items-start">
                      <span className="font-semibold text-purple-300">
                        Invited:{" "}
                        {log.service_providers?.business_name || "Unknown"}
                      </span>
                      <span className="text-xs text-slate-500">
                        {new Date(log.created_at).toLocaleTimeString()}
                      </span>
