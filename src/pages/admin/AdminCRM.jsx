...
export default function AdminCRM() {
  ...
  const { isConnected } = useRealtimeSubscription("crm_leads", {
    onInsert: (newLead) => {
      setLeads((prev) => [newLead, ...prev]);
      toast({
        title: "🎯 New Lead!",
        description: `${newLead.interest || "New inquiry"} from ${newLead.source || "unknown"}`,
      });
    },
  ...
  const getStatusBadge = (status) => {
    switch (status) {
      ...
      case "contacted":
        return (
          <Badge className="bg-yellow-500 hover:bg-yellow-600">Contacted</Badge>
        );
      case "qualified":
        return (
          <Badge className="bg-purple-500 hover:bg-purple-600">Qualified</Badge>
        );
      case "closed":
        return (
    ...
    <div className="min-h-screen bg-slate-950 text-white p-8 space-y-8 font-sans">
      <div>
        ...
        <p className="text-slate-400">
          Manage leads and opportunities from the Concierge
        </p>
      </div>
