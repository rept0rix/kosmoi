...
const AdminSales = () => {
    ...
    const filteredInvitations = invitations.filter(inv => {
        const matchesSearch = inv.service_providers?.business_name.toLowerCase().includes(searchTerm.toLowerCase());
    ...
    const getStatusBadge = (inv) => {
        if (inv.status === 'claimed') return <span className="px-2 py-1 bg-green-500/10 text-green-500 rounded text-xs font-medium">Claimed</span>;
        if (inv.clicked_at) return <span className="px-2 py-1 bg-blue-500/10 text-blue-500 rounded text-xs font-medium">Clicked</span>;
        if (inv.opened_at) return <span className="px-2 py-1 bg-purple-500/10 text-purple-500 rounded text-xs font-medium">Opened</span>;
        return <span className="px-2 py-1 bg-gray-500/10 text-gray-400 rounded text-xs font-medium">Sent</span>;
    };
        ...
        <div className="p-6 space-y-6 bg-[#0a0a0a] min-h-screen text-white">
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-gray-400 text-sm mt-1">Track Sarah's automated outreach performance</p>
                </div>
                <button
