...
import {
  agents,
  syncAgentsWithDatabase,
} from "@/features/agents/services/AgentRegistry";
...
import {
  WORKFLOWS,
  workflowService,
} from "@/features/agents/services/WorkflowService";
...
export function useBoardRoom(options = {}) {
  ...
  useEffect(() => {
    ...
    import("@/services/loops/OptimizerLoop")
      .then(({ OptimizerLoop }) => {
  ...
  useEffect(() => {
    if (
      localBrainEnabled &&
      !localLLM.isReady &&
      !localLLM.isDownloading &&
      !localLLM.isGenerating
    ) {
  ...
  const { result: rxTasks } = useRxQuery("tasks", (collection) => {
    if (!selectedMeeting)
      return collection.find({ selector: { id: "nothing" } }); // Return empty if no meeting
  ...
  const [yoloMode, setYoloMode] = useState(false); // 🔥 YOLO Mode: Auto-approve everything
  ...
  const [activeWorkflowState, setActiveWorkflowState] = useState(
  ...
  useEffect(() => {
    const initAgents = async () => {
      ...
      const { loadDynamicPrompts } =
        await import("@/features/agents/services/AgentRegistry");
    ...
    const fetchMeetings = async () => {
      try {
        ...
        toast({
          ...
          description: "Failed to load meetings. Please check your connection.",
          variant: "destructive",
        });
      } finally {
        setIsLoading(false);
    ...
    const channel = supabase
      ...
      .subscribe((status) => {
        if (status === "SUBSCRIBED") {
          ...
          console.warn(
            `BOARD: Channel closed for messages:${selectedMeeting.id}`,
          );
        } else if (status === "CHANNEL_ERROR" || status === "TIMED_OUT") {
          console.error(
            `BOARD: Subscription ERROR for messages:${selectedMeeting.id}`,
            status,
          );
          toast({
  ...
  const triggerAgentReply = async (forcedAgent = null) => {
    ...
    if (activeWorkflowState && !forcedAgent) {
      ...
      if (currentRole !== "user") {
        selectedAgent = agents.find(
          (a) => a.role === currentRole || a.id === currentRole + "-agent",
        );
      }
    }
    ...
    if (localBrainEnabled) {
      ...
      const replyText = await localLLM.generate([
    ...
    try {
      const response = await getAgentReply(selectedAgent, messages, {
        ...
        images: selectedImage
          ? [{ base64: selectedImage, mimeType: "image/png" }]
          : [],
        config: config,
        workflow: activeWorkflowState?.workflow,
        workflowStep: activeWorkflowState?.currentStep,
        approved: yoloMode, // 🔥 Pass YOLO flag to AgentService (Bypass AgentApprovals)
      });
      ...
      if (response.action) {
        ...
        if (response.action.type === "create_task") {
          ...
          toolRouter(
            "write_code",
            { path: response.action.title, content: response.action.code },
            { userId: user?.id, agentId: selectedAgent.id },
          );
  ...
  // Auto Discuss Effect
  useEffect(() => {
    if (!messages.length || !selectedMeeting || !autoDiscuss || typingAgent)
      return;
    const lastMsg = messages[messages.length - 1];
    if (lastMsg.agent_id !== "HUMAN_USER") {
  ...
  useEffect(() => {
    ...
    const tick = async () => {
      ...
      if (decision && decision.nextSpeakerId) {
        if (
          decision.nextSpeakerId === "ceo-agent" &&
          decision.reason.includes("Daily Standup")
        ) {
  ...
  const confirmCreateMeeting = async () => {
    ...
    try {
      ...
      console.warn(
        "Using LOCAL AND OFFLINE Fallback due to error:",
        error.message,
      );
      toast({
        title: "Offline Mode Activated",
        description:
          "Creating local meeting due to connection/permission error.",
  ...
  const handleCreateTask = async ({ title, description, priority }) => {
    ...
    if (!db) {
      ...
      toast({
        title: "System Error",
        description: "Database is not ready. Please refresh.",
        variant: "destructive",
      });
      return;
    }
  ...
  const handleStartDailyStandup = async () => {
    ...
    try {
      ...
      const { DailyStandupService } =
        await import("@/services/loop/DailyStandupService");
      const reportPrompt = await DailyStandupService.generateMorningReport(
        user?.id,
      );
      ...
      toast({
        title: "Error",
        description: "Failed to start daily standup",
        variant: "destructive",
      });
    }
  };
  ...
  const handleStartOneDollarChallenge = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: "Agents are analyzing...",
      });
    } catch (e) {
  ...
  const handleStartWorkflow = (workflowId) => {
    if (!selectedMeeting) return;
    const state = workflowService.startWorkflow(workflowId, {
      meetingId: selectedMeeting.id,
    });
    setActiveWorkflowState(state);
    const wf =
      WORKFLOWS[
        Object.keys(WORKFLOWS).find((k) => WORKFLOWS[k].id === workflowId)
      ];
  ...
  // Worker Status
  const workerStatusItem = knowledgeItems.find(
    (k) => k.key === "WORKER_STATUS",
  );
