...
export const RxDBProvider = ({ children }) => {
  ...
  const handleHardReset = async (force = false) => {
    if (
      !force &&
      !window.confirm(
        "Are you sure? This will clear all local data and reload the app.",
      )
    )
      return;
      ...
      if ("indexedDB" in window) {
        ...
        const deleteDB = (name) => {
          return new Promise((resolve, reject) => {
            ...
            request.onblocked = () => {
              console.warn(
                `Delete blocked for IDB: ${name} (Close other tabs)`,
              );
              resolve();
            };
          });
          ...
          await Promise.all(
            dbs
              .filter(
                (db) =>
                  db.name &&
                  (db.name.includes("kosmoi") || db.name.includes("rxdb")),
              )
              .map((db) => deleteDB(db.name)),
          );
        } catch (err) {
          console.error(
            "Native IDB list failed, falling back to known names",
            err,
          );
          // Fallback names
          await Promise.all([
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (isPublicRoute) {
        console.log(
          "⚡ Fast Path (RxDB): Public Route -> Unblocking UI Immediately",
        );
        setIsLoading(false);
      }
      ...
      try {
        ...
        if (isMounted) {
          ...
          if (database?._isFallback) {
            console.warn(
              "Received Mock DB. Auto-Recovery disabled for stability debugging.",
            );
            // Stop the loop!
            setError(
              new Error(
                "Database consistency check failed. Please click 'Fix Database' manually.",
              ),
            );
            setIsOfflineMode(true);
            setIsLoading(false);
