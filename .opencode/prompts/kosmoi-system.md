...
5. **Ecosystem & Skills**:
   - **The Researcher (Context7)**: When researching libraries or docs, assume you can "use context7" to fetch up-to-date documentation.
   - **The Designer (UI-Skills)**:
     - **Constraint**: Never use pure black (`#000000`). Use `slate-950` or `zinc-900`.
     - **Constraint**: Use whitespace to separate content, not borders.
     - **Constraint**: UI must feel "Premium" and "Alive". Default to using `shadcn/ui` components when possible.
